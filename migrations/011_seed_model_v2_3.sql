-- 011_seed_model_v2_3.sql
-- GENERATED from dmm-model.js by server/src/scripts/gen-model-migration.ts — do not edit by hand.
-- Seeds model version v2.3 and its capabilities. Idempotent (ON CONFLICT DO NOTHING)
...
VALUES ('v2.3', 'Imported from dmm-model.js')
ON CONFLICT (version) DO NOTHING;

-- Layer 0: Citizens
INSERT INTO capabilities (model_version_id, layer_index, layer_name, layer_covers, order_in_layer, name, measure, includes)
SELECT mv.id, 0, 'Citizens', 'Digital services reaching consumers and households directly', 0, 'Grievance & Feedback', 'Whether the State has the digital capability to systematically collect, analyze, and act upon citizen grievance & feedback regarding rural drinking water services.', ARRAY['Feedback capture', 'Sentiment analysis', 'Closure of loop']::text[]
FROM model_versions mv WHERE mv.version = 'v2.3'
ON CONFLICT (model_version_id, layer_index, order_in_layer) DO NOTHING;

-- v2.3 updates eight capability names and/or measures following terminology clarification.
-- Publish explicitly at UTC midnight; v2.1 and v2.2 remain historical and intact.
UPDATE model_versions
   SET published_at = TIMESTAMPTZ '2026-09-12 00:00:00+00',
       public_notes = 'Updated terminology and assessment wording for eight capabilities, including grievance and feedback, consumer registry, billing and payments, frontline monitoring, financial management, repair and maintenance, and project delivery and service quality.'
 WHERE version = 'v2.3';

INSERT INTO capabilities (model_version_id, layer_index, layer_name, layer_covers, order_in_layer, name, measure, includes)
SELECT mv.id, 0, 'Citizens', 'Digital services reaching consumers and households directly', 1, 'Consumer Registry', 'Whether the State has a digital registry of FHTC / consumers with a unique identifier, and whether this database is authoritative, complete, up-to-date, interoperable and actively used.', ARRAY['Unique consumer ID', 'Household mapping', 'Interoperability', 'Active use']::text[]
FROM model_versions mv WHERE mv.version = 'v2.3'
ON CONFLICT (model_version_id, layer_index, order_in_layer) DO NOTHING;
...
INSERT INTO capabilities (model_version_id, layer_index, layer_name, layer_covers, order_in_layer, name, measure, includes)
SELECT mv.id, 0, 'Citizens', 'Digital services reaching consumers and households directly', 3, 'Billing & Payments', 'Whether the state has the capability to digitally view bills, receive payment reminders, make online payments through multiple channels, access payment history and receive digital receipts.', ARRAY['Bill view', 'Reminders', 'Online payment', 'Payment history', 'Digital receipts']::text[]
FROM model_versions mv WHERE mv.version = 'v2.3'
ON CONFLICT (model_version_id, layer_index, order_in_layer) DO NOTHING;
...
INSERT INTO capabilities (model_version_id, layer_index, layer_name, layer_covers, order_in_layer, name, measure, includes)
SELECT mv.id, 1, 'Frontline Workers', 'Digital tools used by field staff for monitoring and reporting', 0, 'Water Quantity & Regularity', 'Whether the system has the capability to enable frontline workers to accurately measure, digitally report and validate daily water supply data in a timely manner using standardized processes and digital tools.', ARRAY['Daily reporting', 'Validation', 'Integration', 'Governance']::text[]
FROM model_versions mv WHERE mv.version = 'v2.3'
ON CONFLICT (model_version_id, layer_index, order_in_layer) DO NOTHING;

INSERT INTO capabilities (model_version_id, layer_index, layer_name, layer_covers, order_in_layer, name, measure, includes)
SELECT mv.id, 1, 'Frontline Workers', 'Digital tools used by field staff for monitoring and reporting', 1, 'FTK Testing', 'Whether the system has the capability to enable frontline workers to conduct Field Test Kit (FTK) tests, digitally report water quality observations, identify potential quality issues and support routine village-level surveillance.', ARRAY['FTK testing', 'Digital reporting', 'Issue identification']::text[]
FROM model_versions mv WHERE mv.version = 'v2.3'
ON CONFLICT (model_version_id, layer_index, order_in_layer) DO NOTHING;

INSERT INTO capabilities (model_version_id, layer_index, layer_name, layer_covers, order_in_layer, name, measure, includes)
SELECT mv.id, 1, 'Frontline Workers', 'Digital tools used by field staff for monitoring and reporting', 2, 'Repair and Maintenance', 'Whether the system has the capability to effectively monitor minor maintenance and repair activities to ensure timely completion, quality of repairs, asset uptime, and continuous improvement.', ARRAY['Maintenance logs', 'O&M expenditure', 'Collections']::text[]
FROM model_versions mv WHERE mv.version = 'v2.3'
ON CONFLICT (model_version_id, layer_index, order_in_layer) DO NOTHING;
...
INSERT INTO capabilities (model_version_id, layer_index, layer_name, layer_covers, order_in_layer, name, measure, includes)
SELECT mv.id, 1, 'Frontline Workers', 'Digital tools used by field staff for monitoring and reporting', 4, 'Financial Management', 'Whether the system has the capability to enable frontline workers to digitally record O&M expenditure, collections and village-level financial management.', ARRAY['Field collection', 'Inspections', 'Surveys', 'Compliance records']::text[]
FROM model_versions mv WHERE mv.version = 'v2.3'
ON CONFLICT (model_version_id, layer_index, order_in_layer) DO NOTHING;
...
INSERT INTO capabilities (model_version_id, layer_index, layer_name, layer_covers, order_in_layer, name, measure, includes)
SELECT mv.id, 2, 'Department', 'Digital systems for planning, finance, and governance at department level', 6, 'Project Delivery & Service Quality', 'Whether the State has the capability to digitally plan, execute, monitor and validate rural water infrastructure projects, ensuring timely completion, adherence to quality standards, and compliance with contractual and technical requirements.', ARRAY['Planning', 'Execution', 'Quality checks', 'Compliance']::text[]
FROM model_versions mv WHERE mv.version = 'v2.3'
ON CONFLICT (model_version_id, layer_index, order_in_layer) DO NOTHING;

INSERT INTO capabilities (model_version_id, layer_index, layer_name, layer_covers, order_in_layer, name, measure, includes)
SELECT mv.id, 2, 'Department', 'Digital systems for planning, finance, and governance at department level', 7, 'Operations & Resource Management', 'Whether the State has the capability to digitally plan, allocate and monitor operational resources, including workforce, equipment, materials, consumables and finances, to support efficient O&M of water supply schemes.', ARRAY['Workforce', 'Equipment', 'Materials', 'Finances']::text[]
FROM model_versions mv WHERE mv.version = 'v2.3'
ON CONFLICT (model_version_id, layer_index, order_in_layer) DO NOTHING;

-- Layer 3: State Functionaries
INSERT INTO capabilities (model_version_id, layer_index, layer_name, layer_covers, order_in_layer, name, measure, includes)
SELECT mv.id, 3, 'State Functionaries', 'Cross-department coordination and strategic decision-making', 0, 'Interdepartmental Coordination & Collaboration', 'Whether the State has the capability to digitally coordinate, collaborate and exchange information across departments to enable seamless execution of programmes and coordinated governance of rural drinking water services.', ARRAY['Data exchange', 'Workflow', 'Coordination']::text[]
FROM model_versions mv WHERE mv.version = 'v2.3'
ON CONFLICT (model_version_id, layer_index, order_in_layer) DO NOTHING;

INSERT INTO capabilities (model_version_id, layer_index, layer_name, layer_covers, order_in_layer, name, measure, includes)
SELECT mv.id, 3, 'State Functionaries', 'Cross-department coordination and strategic decision-making', 1, 'Integrated Planning & Programme Governance', 'Whether the State has the capability to digitally support joint planning, approvals, programme monitoring and governance across multiple departments.', ARRAY['Joint planning', 'Approvals', 'Programme governance']::text[]
FROM model_versions mv WHERE mv.version = 'v2.3'
ON CONFLICT (model_version_id, layer_index, order_in_layer) DO NOTHING;

INSERT INTO capabilities (model_version_id, layer_index, layer_name, layer_covers, order_in_layer, name, measure, includes)
SELECT mv.id, 3, 'State Functionaries', 'Cross-department coordination and strategic decision-making', 2, 'Cross-Sector Policy & Regulatory Alignment', 'Whether the State has the capability to digitally support alignment of policies, regulations, standards and institutional responsibilities across sectors.', ARRAY['Policy alignment', 'Regulatory coordination']::text[]
FROM model_versions mv WHERE mv.version = 'v2.3'
ON CONFLICT (model_version_id, layer_index, order_in_layer) DO NOTHING;

INSERT INTO capabilities (model_version_id, layer_index, layer_name, layer_covers, order_in_layer, name, measure, includes)
SELECT mv.id, 3, 'State Functionaries', 'Cross-department coordination and strategic decision-making', 3, 'Strategic Intelligence & Decision Support', 'Whether the State has the capability to provide integrated analytics, dashboards and cross-sector insights that enable evidence-based planning, monitoring and strategic decision-making.', ARRAY['Analytics', 'Dashboards', 'Performance monitoring']::text[]
FROM model_versions mv WHERE mv.version = 'v2.3'
ON CONFLICT (model_version_id, layer_index, order_in_layer) DO NOTHING;

INSERT INTO capabilities (model_version_id, layer_index, layer_name, layer_covers, order_in_layer, name, measure, includes)
SELECT mv.id, 3, 'State Functionaries', 'Cross-department coordination and strategic decision-making', 4, 'Emergency Preparedness & Resilience Management', 'Whether the State has the capability to digitally coordinate preparedness, emergency response, disaster management and recovery across departments to ensure continuity of drinking water services.', ARRAY['Disaster management', 'Emergency response', 'Drought', 'Floods', 'Contamination events']::text[]
FROM model_versions mv WHERE mv.version = 'v2.3'
ON CONFLICT (model_version_id, layer_index, order_in_layer) DO NOTHING;

-- Layer 4: Shared Digital Services
INSERT INTO capabilities (model_version_id, layer_index, layer_name, layer_covers, order_in_layer, name, measure, includes)
SELECT mv.id, 4, 'Shared Digital Services', 'Services shared across all stakeholder groups', 0, 'Stakeholder Identity & Lifecycle Management', 'Whether the State has the capability to digitally onboard, authenticate, authorize and manage the lifecycle, profiles and access of all stakeholders across the water service ecosystem.', ARRAY['User management', 'Onboarding', 'Identity', 'Roles', 'Access']::text[]
FROM model_versions mv WHERE mv.version = 'v2.3'
ON CONFLICT (model_version_id, layer_index, order_in_layer) DO NOTHING;

INSERT INTO capabilities (model_version_id, layer_index, layer_name, layer_covers, order_in_layer, name, measure, includes)
SELECT mv.id, 4, 'Shared Digital Services', 'Services shared across all stakeholder groups', 1, 'Capacity Building & Knowledge Management', 'Whether the State has the capability to digitally deliver training, certifications, knowledge resources, SOPs and continuous learning opportunities to strengthen stakeholder competencies.', ARRAY['LMS', 'Training', 'Certification', 'Knowledge', 'Digital learning']::text[]
FROM model_versions mv WHERE mv.version = 'v2.3'
ON CONFLICT (model_version_id, layer_index, order_in_layer) DO NOTHING;

INSERT INTO capabilities (model_version_id, layer_index, layer_name, layer_covers, order_in_layer, name, measure, includes)
SELECT mv.id, 4, 'Shared Digital Services', 'Services shared across all stakeholder groups', 2, 'Personalized Dashboards & Reports', 'Whether the State has the capability to provide stakeholders with role-based dashboards, reports, alerts and insights that support operational monitoring, performance management and informed decision-making.', ARRAY['Role-based dashboards', 'Personalized KPIs', 'Drill-down', 'Self-service reporting', 'Automated distribution']::text[]
FROM model_versions mv WHERE mv.version = 'v2.3'
ON CONFLICT (model_version_id, layer_index, order_in_layer) DO NOTHING;

INSERT INTO capabilities (model_version_id, layer_index, layer_name, layer_covers, order_in_layer, name, measure, includes)
SELECT mv.id, 4, 'Shared Digital Services', 'Services shared across all stakeholder groups', 3, 'Intelligent Assistance & Decision Support', 'Whether the State has the capability to provide contextual assistance, conversational interfaces, recommendations and guided workflows that help stakeholders perform tasks more effectively.', ARRAY['Context-aware guidance', 'Decision support', 'Recommendations', 'Conversational assistance']::text[]
FROM model_versions mv WHERE mv.version = 'v2.3'
ON CONFLICT (model_version_id, layer_index, order_in_layer) DO NOTHING;

INSERT INTO capabilities (model_version_id, layer_index, layer_name, layer_covers, order_in_layer, name, measure, includes)
SELECT mv.id, 4, 'Shared Digital Services', 'Services shared across all stakeholder groups', 4, 'Omnichannel Digital Experience', 'Whether the State has the capability to enable stakeholders to access digital services through multiple channels such as mobile applications, web portals, WhatsApp, SMS, IVRS and future digital interfaces.', ARRAY['WhatsApp', 'Mobile', 'Portal', 'IVRS', 'SMS', 'Email']::text[]
FROM model_versions mv WHERE mv.version = 'v2.3'
ON CONFLICT (model_version_id, layer_index, order_in_layer) DO NOTHING;

-- Layer 5: Technology Foundation
INSERT INTO capabilities (model_version_id, layer_index, layer_name, layer_covers, order_in_layer, name, measure, includes)
SELECT mv.id, 5, 'Technology Foundation', 'The software layer underlying every other layer', 0, 'Digital Platform & Architecture', 'Whether the State has the capability to build and operate scalable, modular, resilient and maintainable digital solutions using modern architectural principles, reusable components and cloud-ready technologies.', ARRAY['Unified platform', 'Modular architecture', 'Microservices', 'Cloud native', 'DevOps', 'Containerization']::text[]
FROM model_versions mv WHERE mv.version = 'v2.3'
ON CONFLICT (model_version_id, layer_index, order_in_layer) DO NOTHING;

INSERT INTO capabilities (model_version_id, layer_index, layer_name, layer_covers, order_in_layer, name, measure, includes)
SELECT mv.id, 5, 'Technology Foundation', 'The software layer underlying every other layer', 1, 'Interoperability & Open Integration', 'Whether the State has the capability to seamlessly exchange information and integrate with central, state and external systems through open standards, APIs, common data models and interoperable interfaces.', ARRAY['APIs', 'Centre systems', 'State systems', 'Standards', 'Registries', 'Eventing']::text[]
FROM model_versions mv WHERE mv.version = 'v2.3'
ON CONFLICT (model_version_id, layer_index, order_in_layer) DO NOTHING;

INSERT INTO capabilities (model_version_id, layer_index, layer_name, layer_covers, order_in_layer, name, measure, includes)
SELECT mv.id, 5, 'Technology Foundation', 'The software layer underlying every other layer', 2, 'Data Management & Governance', 'Whether the State has the capability to establish trusted, high-quality and well-governed data through standardized data models, master data management, metadata, data quality and data governance practices.', ARRAY['Master data management', 'Metadata', 'Data quality', 'Data governance', 'Data catalog', 'Lineage', 'Stewardship']::text[]
FROM model_versions mv WHERE mv.version = 'v2.3'
ON CONFLICT (model_version_id, layer_index, order_in_layer) DO NOTHING;

INSERT INTO capabilities (model_version_id, layer_index, layer_name, layer_covers, order_in_layer, name, measure, includes)
SELECT mv.id, 5, 'Technology Foundation', 'The software layer underlying every other layer', 3, 'Digital Identity & Access Management', 'Whether the State has the capability to securely manage digital identities, authentication, authorization and access control for users, applications and services while ensuring privacy and accountability.', ARRAY['IAM', 'SSO', 'RBAC', 'MFA']::text[]
FROM model_versions mv WHERE mv.version = 'v2.3'
ON CONFLICT (model_version_id, layer_index, order_in_layer) DO NOTHING;

INSERT INTO capabilities (model_version_id, layer_index, layer_name, layer_covers, order_in_layer, name, measure, includes)
SELECT mv.id, 5, 'Technology Foundation', 'The software layer underlying every other layer', 4, 'Intelligent Automation & Decision Support', 'Whether the State has the capability to leverage intelligent technologies to automate business processes, generate insights, support decision-making and improve operational efficiency.', ARRAY['AI', 'ML', 'OCR', 'Prediction', 'Recommendations', 'NLP']::text[]
FROM model_versions mv WHERE mv.version = 'v2.3'
ON CONFLICT (model_version_id, layer_index, order_in_layer) DO NOTHING;

INSERT INTO capabilities (model_version_id, layer_index, layer_name, layer_covers, order_in_layer, name, measure, includes)
SELECT mv.id, 5, 'Technology Foundation', 'The software layer underlying every other layer', 5, 'Security, Privacy & Compliance', 'Whether the State has the capability to protect digital systems and data through comprehensive security controls, privacy safeguards, risk management and compliance with applicable standards and regulations.', ARRAY['Security', 'Privacy', 'Compliance', 'Audit', 'Encryption', 'Monitoring']::text[]
FROM model_versions mv WHERE mv.version = 'v2.3'
ON CONFLICT (model_version_id, layer_index, order_in_layer) DO NOTHING;
