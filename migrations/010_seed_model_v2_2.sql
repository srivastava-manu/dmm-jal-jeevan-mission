-- 010_seed_model_v2_2.sql
-- GENERATED from dmm-model.js by server/src/scripts/gen-model-migration.ts — do not edit by hand.
-- Seeds model version v2.2 and its capabilities. Idempotent (ON CONFLICT DO NOTHING)
...
-- v2.2 has an eight-capability Department layer. Keep the historical six-capability
-- model rows intact while widening the schema for future uneven layer sizes.
ALTER TABLE capabilities DROP CONSTRAINT IF EXISTS capabilities_order_in_layer_check;
ALTER TABLE capabilities
  ADD CONSTRAINT capabilities_order_in_layer_check CHECK (order_in_layer BETWEEN 0 AND 7);

INSERT INTO model_versions (version, notes)
VALUES ('v2.2', 'Imported from dmm-model.js')
...
FROM model_versions mv WHERE mv.version = 'v2.2'
...
-- v2.2 is the revised model received from State feedback. Publish explicitly at UTC midnight.
UPDATE model_versions
   SET published_at = TIMESTAMPTZ '2026-09-11 00:00:00+00',
       public_notes = 'Revised following State feedback: the Infrastructure Foundation and Agencies layers were removed; selected agency capabilities were retired or moved into Department; and two capabilities were removed from State Functionaries and Shared Digital Services.'
 WHERE version = 'v2.2';
